...
import org.loader.dto.GitHubDataDto;
...
/**
 * Parser for GitHub CSV files.
 * Converts GitHub-specific CSV format to PlatformInformation entities.
 */
@Component
@Slf4j
public class GitHubParser implements DataParser {

    @Override
    public List<PlatformInformation> parse(File file) {
        log.info("Parsing GitHub file: {}", file.getName());

        try (FileReader reader = new FileReader(file)) {
            // Parse CSV using OpenCSV
            List<GitHubDataDto> dtos = new CsvToBeanBuilder<GitHubDataDto>(reader)
                    .withType(GitHubDataDto.class)
                    .withIgnoreLeadingWhiteSpace(true)
                    .withIgnoreEmptyLine(true)
                    .build()
                    .parse();

            log.info("Found {} records in GitHub file: {}", dtos.size(), file.getName());
            ...
            log.info("Successfully transformed {} out of {} records from GitHub file",
                    entities.size(), dtos.size());

            return entities;

        } catch (Exception e) {
            log.error("Failed to parse GitHub file: {}", file.getName(), e);
            throw new FileParsingException(
                    "Failed to parse GitHub file: " + e.getMessage(),
                    file.getName(),
                    e
            );
        }
    ...
    /**
     * Transform GitHub DTO to PlatformInformation entity.
     * Maps GitHub-specific field names to database columns.
     */
    private PlatformInformation transform(GitHubDataDto dto) {
        // Parse label
        ...
        return PlatformInformation.builder()
                ...
                .developerId(dto.getDevloperId())    // devloper_id → developer_id
                .taskNumber(dto.getIssue())          // issue → task_number
                .environment(dto.getEnvironment())   // environment → environment
                ...
                .provider(Provider.GITHUB)           // Set provider
                .build();
    }
